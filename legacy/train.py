...
def train():
    ...
    train_loader, val_loader, _ = get_dataloaders(test_size=0.1, val_size=0.1)
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        for x_batch, y_batch in train_loader:
            ...
            loss.backward()
            optimizer.step()
            # scheduler.step() batch step if using one cycle LR
        ...
        with torch.no_grad():
            for x_val, y_val in val_loader:
                ...
                # get raw sin/cos predictions and labels
                y_pred_np = y_pred.cpu().numpy()  # shape (B,6)
                y_val_np  = y_val .cpu().numpy()  # shape (B,6)
                ...
                # compute manipulability on recovered angles
        ...
        scheduler.step() # epoch step if using cosine annealing
